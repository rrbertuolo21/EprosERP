...
/**
 * NfeProdutosCard — lista de itens da NF-e (porta NfeProdutosCard do legado).
 *
 * Renderiza a tabela de produtos (código, descrição, NCM, CFOP, unidade, quantidade, unitário,
 * desconto, total, %ICMS, %IPI) com ações de editar/remover por linha e o botão de adicionar.
 * Não faz IO — apenas emite eventos que a página trata via `useNfeEmissao`.
 */
import { computed } from 'vue'
import { useHelper } from '~/composables/useHelper'
import type { NfeItem } from './nfeTypes'

const props = defineProps<{
  itens: NfeItem[]
  /** Desabilita a inclusão quando não há destinatário. */
  bloqueado?: boolean
  /** Somente leitura (nota já autorizada). */
  readonly?: boolean
  erro?: string
}>()
  ...
  <section class="glass-panel nfe-card">
    <header class="nfe-card-header">
      ...
      <button
        ...
        :disabled="bloqueado"
        :title="bloqueado ? 'Selecione um destinatário para adicionar produtos' : ''"
