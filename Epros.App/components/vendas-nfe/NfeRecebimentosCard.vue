...
/**
 * NfeRecebimentosCard — formas de pagamento e duplicatas (porta NfeRecebimentosCard do legado).
 *
 * Permite adicionar/remover pagamentos (modalidade + valor + troco) e duplicatas da fatura
 * (número + vencimento + valor). Mostra o total de recebimento vs. total da nota, com aviso de
 * diferença. Sem cálculo server-side: apenas coleta os dados para o payload.
 */
...
const props = defineProps<{
  modelValue: NfeForm
  /** Total da nota, para conferência do recebimento. */
  totalNota: number
  readonly?: boolean
}>()
...
const totalRecebido = computed(() => form.value.pagamentos.reduce((a, p) => a + (p.valorPagamento || 0), 0))
const totalDuplicatas = computed(() => form.value.duplicatas.reduce((a, d) => a + (d.valor || 0), 0))
const diferenca = computed(() => props.totalNota - totalRecebido.value)

function atualizar(patch: Partial<NfeForm>) {
...
<template>
  <section class="glass-panel nfe-card">
    <header class="nfe-card-header">
      <h2 class="nfe-card-title">Recebimentos</h2>
    </header>
    ...
    <div class="rec-bloco">
      ...
      <div v-for="p in form.pagamentos" :key="p._uid" class="rec-linha">
        ...
        />
        <button v-if="!readonly" type="button" class="btn btn-ghost btn-sm" @click="removerPagamento(p._uid)">🗑</button>
      </div>
    </div>
    ...
    <div class="rec-bloco">
      <div class="rec-bloco-header">
        <h3 class="rec-subtitulo">Duplicatas (fatura)</h3>
      ...
      <div v-for="d in form.duplicatas" :key="d._uid" class="rec-linha rec-linha-dup">
        ...
        />
        <button v-if="!readonly" type="button" class="btn btn-ghost btn-sm" @click="removerDuplicata(d._uid)">🗑</button>
      </div>
    </div>
    ...
    <div class="rec-resumo">
      ...
      <div><span>Total recebido</span><strong>{{ formatarMoeda(totalRecebido) }}</strong></div>
...
.rec-resumo {
  ...
  padding: 12px 14px; border-radius: 8px; background: rgba(255, 255, 255, 0.02);
...
.rec-dif strong { color: var(--warning, #f59e0b); }
