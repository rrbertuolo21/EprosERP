...
/**
 * NF-e de Entrada — Devolução/Retorno (fatia 13)
 * `erp/compras/emissao/devolucao-retorno/nfe-entrada/[[id]]`.
 *
 * Porta o comportamento da tela legada `compras/emissao/devolucao-retorno/nfe-entrada/[[id]].vue`
 * (que usava `useNfeEmissaoPage(nfeEntradaDevolucaoRetornoConfig)`). É a mesma NF-e de entrada,
 * porém obrigatoriamente referenciando a(s) NF-e de origem (devolução/retorno de mercadoria).
 *
 * Reutiliza os componentes locais da fatia e o composable `useNfeEntrada('devolucao')`,
 * que ajusta a natureza padrão e torna as chaves referenciadas obrigatórias na validação.
 *
 * Endpoints: compras (lancar, {id}, {id}/cancelar); vendas-fiscal/{id}/nfe/referenciadas
 * (quando existir no backend); vendas-fiscal (DANFE, quando existir).
 */
...
async function carregarCfops() {
  try {
    ...
    console.error('[nfe-entrada-devolucao] falha ao carregar CFOPs', e)
  }
}
...
const titulo = computed(() =>
  nfe.id
    ? `Edição de NF-e de Entrada (Devolução/Retorno) ${nfe.numero ? `#${nfe.numero}` : ''}`.trim()
    : 'NF-e de Entrada — Devolução/Retorno'
)
...
async function onNovaNota() {
  ...
  if (ok) router.push('/erp/compras/emissao/devolucao-retorno/nfe-entrada')
}
...
<template>
  <div class="entrada-page">
    <PageToolbar :title="titulo" subtitle="Compras · Devolução/Retorno · NF-e de Entrada" :loading="carregando">
      <template #actions>
    ...
    <!-- Aviso de devolução/retorno -->
    <div class="entrada-aviso glass-panel">
      <span class="chip chip-warning">Devolução / Retorno</span>
      <span class="entrada-aviso-txt">
        Referencie a(s) NF-e de origem para vincular esta devolução/retorno de mercadoria.
      </span>
      <button type="button" class="btn btn-secondary btn-sm" @click="mostrarReferenciadas = true">
        Referenciar NF-e
        <span v-if="nfe.chavesReferenciadas.length" class="chip chip-info">{{ nfe.chavesReferenciadas.length }}</span>
      </button>
    </div>
    ...
    <div v-if="nfe.situacao" class="entrada-status-bar glass-panel">
      ...
      <span class="chip" :class="{ 'chip-success': lancada, 'chip-danger': cancelada }">{{ nfe.situacao }}</span>
    ...
    <EntradaInformacoesCard
      ...
      :erro-referenciadas="erros.referenciadas"
...
.entrada-aviso-txt { flex: 1; font-size: 13px; color: var(--text-secondary); min-width: 220px; }
.entrada-status-bar {
  display: flex; align-items: center; flex-wrap: wrap; gap: 12px;
  padding: 12px 18px; margin-bottom: 16px;
}
