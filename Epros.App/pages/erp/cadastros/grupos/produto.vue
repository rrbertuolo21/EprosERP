...
/**
 * Grupo de Produtos — Cadastros / Grupos.
 *
 * Porta o comportamento de `cadastros/grupo/produto.vue` do legado:
 * listagem simples (id + descrição) com modal de criação/edição e exclusão.
 * Endpoint: `produto-grupos`.
 */
...
const lista = useApiList<ProdutoGrupo>('/produto-grupos', {
  tamanhoPaginaInicial: 20
})

const colunas: DataTableColumn<ProdutoGrupo>[] = [
...
const form = reactive<ProdutoGrupo>({ id: 0, descricao: '' })
const erroDescricao = ref('')

const excluirVisivel = ref(false)
const excluindo = ref(false)
const itemParaExcluir = ref<ProdutoGrupo | null>(null)
...
function abrirEdicao(item: ProdutoGrupo) {
  editando.value = true
  form.id = item.id
...
async function salvar() {
  ...
  try {
    if (editando.value) {
      await useApi(`/produto-grupos/${form.id}`, { method: 'PUT', body: { id: form.id, descricao: form.descricao } })
    } else {
      await useApi('/produto-grupos', { method: 'POST', body: { descricao: form.descricao } })
    }
...
function pedirExclusao(item: ProdutoGrupo) {
...
async function confirmarExclusao() {
  ...
  try {
    await useApi(`/produto-grupos/${itemParaExcluir.value.id}`, { method: 'DELETE' })
...
<template>
  <div>
    <PageToolbar title="Grupo de Produtos" subtitle="Agrupamentos usados na classificação de produtos" :loading="lista.carregando.value">
      <template #actions>
    ...
    <DataTable
      ...
      empty-text="Nenhum grupo de produtos cadastrado."
    ...
    <AppDialog v-model="dialogVisivel" :title="editando ? 'Editando' : 'Novo Grupo'" width="480px">
