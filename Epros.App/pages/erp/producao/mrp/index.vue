...
/**
 * Listagem de Planejamentos MRP/IBP — Produção / MRP.
 * GET lista (+ filtro status) + GET/{id} + POST criar + workflow. Sem PUT/DELETE.
 * Fonte: ProducaoMrpController (api/v1/producao/mrp/planejamentos) + MrpPlanejamentoQueries.
 */
...
interface MrpPlanejamento {
  id: string
  codigo?: string | null
  status?: number | string | null
  criadoEm?: string | null
}
interface MrpFiltros { status?: string | null }

const router = useRouter()
const lista = useApiList<MrpPlanejamento, MrpFiltros>('/producao/mrp/planejamentos', {
  filtrosIniciais: { status: null },
  tamanhoPaginaInicial: 20
})

const colunas: DataTableColumn<MrpPlanejamento>[] = [
...
function novo() { router.push('/erp/producao/mrp/novo') }
function abrir(item: MrpPlanejamento) { router.push(`/erp/producao/mrp/${item.id}`) }
...
<template>
  <div>
    <PageToolbar title="Planejamento MRP/IBP" subtitle="Planejamento de necessidades de materiais" :loading="lista.carregando.value">
      <template #actions>
    ...
    <DataTable
      ...
      :loading="lista.carregando.value"
      empty-text="Nenhum planejamento MRP encontrado."
