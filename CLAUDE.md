...
> Onboarding humano: [COMECE-AQUI.md](COMECE-AQUI.md). Processo completo: `_fabrica/`.
...
`src/` backend · `Epros.App/` front · `scripts/` · `infra/` · `docker-compose.{local,prod}.yml` ·
`tests/` · `Epros.sln`. Módulos em `src/Modules/Epros.Modules.<X>/`. Diário: `HISTORICO-DESENVOLVIMENTO-IA.md`.
...
1. **O "verde" é entrada, não prova.** Nunca reporte "build passou/testes verdes" sem **re-executar**
   ...
   **ambiente vivo** (banco real, chamada externa real): pode faltar migration, ter URL hardcoded, ou
   faltar adaptador de integração. Detalhe: `_fabrica/processo/RETROSPECTIVA-EPROSERP.md`.
2. **Produção paralela = fan-out por pasta disjunta.** Muitas telas/módulos? 1 agente por pasta
   ...
   multiplicar, contrato fixo entre paralelos. Ver `_fabrica/skills/fan-out-paralelo.md`.
3. **Fidelidade campo-a-campo** à EF/legado. Nada fakeado. Porte fecha só com **migration aplicada +
   CRUD completo por raiz** (GET/{id}, PUT, DELETE) + detalhe. Ver `_fabrica/skills/armadilhas-de-porte.md`.
...
6. **Convenções de código:** `CONVENCAO_CODIGO.md` (Modo Porte vs Consolidação) + regras `.mdc` do
   Cursor em `_fabrica/cursor/cursor-install/rules/`.
...
`--no-deps` rebuilda api/web sem derrubar o banco. Ver `_fabrica/skills/docker-deploy-armadilhas.md`.

## Onde está o processo (para a IA carregar)
- Agentes (Dev, QA, Arquiteto, Segurança, Code Review, Eng. Reversa, Context): `_fabrica/agentes/`
- Processo/gate/fan-out: `_fabrica/processo/` · Skills destiladas: `_fabrica/skills/`
