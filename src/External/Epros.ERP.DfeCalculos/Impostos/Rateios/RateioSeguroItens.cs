...
{
    public class RateioSeguroItens
    {
        public static ICollection<VendaItem> Ratear(decimal valorTotalParaRateio, ICollection<VendaItem> itens)
        {
            ...
            {
                itensTributaveis.ToList().ForEach(item => item.DefinirValorSeguroRatiado(decimal.Zero));
            ...
            foreach (var item in itensTributaveis)
            {
                ...
                item.DefinirValorSeguroRatiado(valorProporcional.Arredonda2());
            }

            var somaDoRateioNosItens = itensTributaveis.ToList().Sum(x => x.ValorSeguroRateado);
            ...
            ultimoItem.DefinirValorSeguroRatiado(ultimoItem.ValorSeguroRateado + valorASerCorrigido);
