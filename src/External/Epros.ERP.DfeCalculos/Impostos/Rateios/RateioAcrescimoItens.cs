...
{
    public class RateioAcrescimoItens
    {
        public static ICollection<VendaItem> Ratear(decimal valorTotalParaRateio, ICollection<VendaItem> itens)
        {
            ...
            {
                itensTributaveis.ToList().ForEach(item => item.DefinirValorAcrescimoRatiado(decimal.Zero));
            ...
            foreach (var item in itensTributaveis)
            {
                ...
                item.DefinirValorAcrescimoRatiado(valorProporcional.Arredonda2());
            }

            var somaDoRateioNosItens = itensTributaveis.ToList().Sum(x => x.ValorAcrescimoRateado);
            ...
            ultimoItem.DefinirValorAcrescimoRatiado(ultimoItem.ValorAcrescimoRateado + valorASerCorrigido);
