...
namespace Epros.API.Controllers
{
    ...
    public class RevendasController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new ListarRevendasQuery(pagina, tamanhoPagina, search));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new ObterRevendaPorIdQuery(id));
        ...
        public async Task<ActionResult<CommandResult>> Create([FromBody] CriarRevendaCommand command)
        {
        ...
        [ProducesResponseType(typeof(CommandResult), StatusCodes.Status422UnprocessableEntity)]
        public async Task<IActionResult> Update(Guid id, [FromBody] AtualizarRevendaCommand command)
        {
            if (id != command.Id)
            {
        ...
        {
            var result = await _mediator.Send(new ExcluirRevendaCommand(id));
            if (!result.Sucesso) return UnprocessableEntity(result);
            return Ok(result);
        }
    }
