...
namespace Epros.API.Controllers
{
    ...
    public class ContadoresController : ControllerBase
    {
        ...
        public ContadoresController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var query = new ListarContadoresQuery(localizar, pagina, tamanhoPagina);
        ...
        {
            var query = new ObterContadorPorIdQuery(id);
            var result = await _mediator.Send(query, cancellationToken);
            if (!result.Sucesso)
            {
        ...
        public async Task<IActionResult> Criar([FromBody] CriarContadorCommand command, CancellationToken cancellationToken)
        {
        ...
        [ProducesResponseType(typeof(CommandResult), StatusCodes.Status422UnprocessableEntity)]
        public async Task<IActionResult> Atualizar(Guid id, [FromBody] AtualizarContadorCommand command, CancellationToken cancellationToken)
        {
            if (id != command.Id)
            {
        ...
        {
            var command = new DeletarContadorCommand(id);
            var result = await _mediator.Send(command, cancellationToken);
            if (!result.Sucesso)
            {
                return UnprocessableEntity(result);
