    ...
    public class PermissaoMenuFilter : IAsyncAuthorizationFilter
    {
        ...
        public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
        {
            ...
            if (!string.IsNullOrEmpty(authHeader))
            {
                ...
                if (token.StartsWith("jwt-token-completo-"))
                {
                    var partes = token.Split('-');
                    // jwt-token-completo-{tenantId}-{usuarioId}-{empresaId}-{perfilId}-{guid}
                    // Ex: jwt-token-completo-tenant1-usr1-emp1-perfil1-uuid
                    // Partes do split:
                    // 0: jwt
                    // 1: token
                    // 2: completo
                    // 3: tenantId (se contiver '-' pode quebrar, mas nos tokens que geramos é estruturado sem '-')
                    // Para ser resiliente, podemos fazer uma extração mais inteligente baseada em índice reverso ou regex
                    // Nosso token gerado: $"jwt-token-completo-{tenantId}-{usuarioId}-{empresaId}-{vinculo.PerfilUsuarioId}-{Guid.NewGuid()}"
                    // Se o tenantId contiver hifens, vamos extrair buscando a palavra "completo"
                    ...
                    if (indexCompleto != -1 && partes.Length > indexCompleto + 3)
                    {
                        ...
                        if (Guid.TryParse(partes[indexCompleto + 2], out var parsedUid)) usuarioId = parsedUid;
                        if (Guid.TryParse(partes[indexCompleto + 3], out var parsedEid)) empresaId = parsedEid;
                    }
                }
            }
