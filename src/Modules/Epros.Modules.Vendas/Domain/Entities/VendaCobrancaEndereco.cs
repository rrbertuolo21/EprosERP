...
{
    /// <summary>
    /// Porte fiel de VendaCobrancaEndereco (endereço de cobrança da venda). FK long -> Guid;
    /// VOs Documento/CEP -> string?; herda EntidadeSaaSBase. EnderecoId referencia
    /// endereço de outro módulo por Guid FK.
    /// </summary>
    public class VendaCobrancaEndereco : EntidadeSaaSBase
    {
        ...
        protected VendaCobrancaEndereco() { } // EF Core

        public VendaCobrancaEndereco(Guid vendaId, string nome, string fone, string email, string? ie, string? documento, EEstado uf, string? logradouro, string? numero, string? complemento, string? bairro, int municipioId, string? municipioNome, string? cep, int paisId, string? paisNome, Guid? enderecoId,
