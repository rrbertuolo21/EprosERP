...
/**
 * EntradaTransporteCard — modalidade de frete e transportadora da NF-e de entrada.
 *
 * Porta o card "Transporte" do legado (`compras/emissao/nfe-entrada`) para a fatia 13:
 * modalidade de frete via select + busca/seleção de transportadora (cadastros/pessoas).
 * Espelha `vendas-nfe/NfeTransporteCard.vue`, adaptado ao `EntradaForm` (campo
 * `transportadora`/`modalidadeFrete` já existem no modelo — só faltava o card na tela).
 */
...
async function buscarTransportadoras(texto: string) {
  ...
  try {
    ...
    console.error('[EntradaTransporteCard] falha na busca de transportadoras', e)
    resultados.value = []
  } finally {
...
.transp-opcao:hover { background: var(--hover-bg); }
