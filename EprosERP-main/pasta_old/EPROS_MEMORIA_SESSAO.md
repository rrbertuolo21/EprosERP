...
| **Repositório desktop** | `Epros.App/` (a criar — Bloco 9) |
| **Repositório mobile** | `Epros.Mobile/` (a criar — Bloco 10) |
...
- Use nomes claros para classes, métodos, variáveis, arquivos e pastas.
...
* **Bloco 5 (Concluído)**: Solução do zero estruturada com 4 projetos (`Shared`, `Infrastructure`, `API`, `GestaoClientes`), pipeline de 7 middlewares HTTP configurado, resolvendo tenant a partir do JWT/Header. Fluxos de Criar, Ativar e Suspender cliente concluídos com 7 testes unitários passando.
* **Bloco 6 (Próximo - Pendente)**: Reengenharia de Estoque/Compras (Compra + CompraItem + Nota de Entrada + ContaPagar + Entrada de Estoque).

### 2. Backlog Técnico
* Provisionar infraestrutura local no `docker-compose.yml` (PostgreSQL 16, Keycloak, Valkey, MinIO, Vault).
* Configurar migrations do EF Core para o `ContextGestaoClientes` no PostgreSQL.
* Configurar o Outbox Worker local (Quartz.NET) para processamento de eventos.
* Ativar segunda barreira de multi-tenancy usando RLS (Row-Level Security) no PostgreSQL.

### 3. Backlog Funcional
* Mapear entidades do núcleo transversal em `plataforma.*`: `plataforma.pessoas`, `plataforma.produtos`, `plataforma.empresas`.
* Implementar commands `CriarPlano`, `GerarFatura`, `BaixarFatura` para fechar o ciclo de faturamento de inquilinos.
* Reengenharia do módulo Financeiro (Contas a Pagar e Contas a Receber).
...
### 5. Pendências
* Configuração do container local de banco de dados para testes integrados.
* Acoplamento com Keycloak real para validação do JWT OIDC.

### 6. Débitos Técnicos
* Mocks temporários de `ICurrentUser` e `ITenantProvider` baseados em claims opcionais de dev para evitar bloqueio local sem Keycloak de pé.

### 7. Testes Pendentes
* Testes de integração utilizando `Testcontainers` para validar o isolamento `QueryFilter` entre tenants no banco real.
* Testes de concorrência e carga no processador de Outbox.

### 8. Riscos Conhecidos
* **Vazamento de dados por Tenant Leak**: Mitigado por `QueryFilter` e RLS, mas requer auditoria constante nas rotas de queries complexas.
* **Queda do provedor de identidade (Keycloak)**: Tratado com cache local de connection routing, mas representa ponto crítico de falha em produção.
