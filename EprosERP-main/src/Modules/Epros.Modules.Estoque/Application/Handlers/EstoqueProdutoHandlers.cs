...
namespace Epros.Modules.Estoque.Application.Handlers
{
    ...
    public class DeletarEstoqueProdutoCommandHandler : ICommandHandler<DeletarEstoqueProdutoCommand>
    {
        ...
        public async Task<CommandResult> Handle(DeletarEstoqueProdutoCommand request, CancellationToken cancellationToken)
        {
            ...
            // MVM-011 / VAL-MVM-007: não excluir saldo quando houver ficha de entrada.
            var possuiFicha = await _context.ProdutoFichaEstoqueEntradas
                .AnyAsync(f => f.EmpresaId == estoque.EmpresaId && f.ProdutoId == estoque.ProdutoId, cancellationToken);
            if (possuiFicha)
                return CommandResult.Falha("Não é possível excluir o saldo: existem fichas de entrada para este produto (MVM-011).");
