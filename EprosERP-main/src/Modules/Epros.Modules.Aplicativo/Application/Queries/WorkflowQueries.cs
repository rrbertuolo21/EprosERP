...
namespace Epros.Modules.Aplicativo.Application.Queries
{
    ...
    public record ListarWfInstanciasQuery(EWfInstanciaStatus? Status, string? EntidadeTipo, Guid? ResponsavelUsuarioId, int Pagina = 1, int TamanhoPagina = 20, DateTime? DesdeUtc = null, DateTime? AteUtc = null) : IQuery<IEnumerable<WfInstanciaDto>>;
    ...
    public class ListarWfInstanciasQueryHandler : IQueryHandler<ListarWfInstanciasQuery, IEnumerable<WfInstanciaDto>>
    {
        ...
        public async Task<IEnumerable<WfInstanciaDto>> Handle(ListarWfInstanciasQuery request, CancellationToken cancellationToken)
        {
            ...
            if (request.DesdeUtc.HasValue) q = q.Where(i => i.CriadoEm >= request.DesdeUtc.Value);
            if (request.AteUtc.HasValue) q = q.Where(i => i.CriadoEm <= request.AteUtc.Value);
